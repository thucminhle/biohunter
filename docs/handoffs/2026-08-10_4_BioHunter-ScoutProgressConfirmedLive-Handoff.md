# BioHunter — 4b Fully Confirmed Live, Cleanup, Scribe Investigation

**Session date:** 2026-08-10 (fourth session this date — continuation of
`2026-08-10_BioHunter-4b-BrowserConfirmed-ScoutProgress-Handoff.md`,
which this doc supersedes as the "read first" pointer).

**One-line summary:** Everything 4b touched is now confirmed working
live, not just written. The `on_company_done` wiring from last session
was re-tested with a real "Run Scout" click and produced correct
per-company progress and a correct final summary end to end. Two real
open items remain, both genuinely blocked on something outside this
chat: Scribe Therapeutics needs a browser's Network tab, and a Guardant
Health rationale check was handed over twice now and never run.
...
**`detector.py` uploaded, and it already solved the Scout-progress
...
company finishes. **Confirmed live, same session, via a real "Run
Scout" click**: took 5+ minutes to move past "0 of 10" (the first
company alphabetically was slow — likely a large Workday-style
postings count plus rate-limiter delay, not a hang), then completed
correctly: "10 companies checked, 0 new postings, 1 error" — a correct
final summary, matching the known Scribe Therapeutics 404 as the sole
error. Worth remembering as a pattern for next time something looks
"stuck" at 0: check for terminal log activity or `curl
localhost:5050/jobs/<id>.json` directly before assuming a hang — one
slow first company can look identical to a stall for several minutes.
...
- **Scribe Therapeutics** — needs someone to open
  ...
  search/fetch alone (JS-rendered content). This is the one item that's
  been narrowed all the way down to "needs eyes on a browser," not more
  digging.
...
- Carried forward from three sessions ago now, still unresolved: the
  Guardant Health "Sr. Client Services Specialist" posting's rationale
  claimed a location mismatch on a posting that had already passed the
  location filter. A query was handed to the project owner twice now to
  check the posting's real stored location — never reported back. Worth
  closing this loop before trusting location-reasoning across the other
  191 rationales at scale.

---

## 4. Recommended files to upload next session

Core (current state, confirmed working):
```
src/biohunter/dashboard.py     (on_company_done wired AND confirmed live)
